...
void lcdIlkAyarlar(){
    ...
    // ayarlar devam
    // lcd komut kullan
    lcdKomut(0x07); // giriþ kipi     // Giriþ modu ayarý (soldan saða)
    lcdKomut(0x0E); // Ekran aç kapa  // Ekran açýk, imleç kapalý
    lcdKomut(0x1c); // Kursör kaydýr
...
void lcdGit(unsigned char row, unsigned char col){
    ...
    //col -= 1; // Sütun numarasini dogrudan girilmesi icin
    if (row == 1) {
